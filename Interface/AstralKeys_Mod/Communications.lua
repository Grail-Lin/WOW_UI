...
-- this is announce all chars keys
...
--			SendChatMessage(strformat('%s-%s %s', addon.CharacterName(i), addon.CharacterRealm(i), link), channel)
...
local function ParseGuildChatCommands(text)
	...
	-- allkeys will show each key of all characters
	if text:lower() == '!allkeys' then
		local guild = GetGuildInfo('player')
		if AstralKeysSettings.general.report_on_message['guild'] then
			addon.AnnounceCharacterKeys('GUILD')
		end
	end
end
...
local function ParsePartyChatCommands(text)
	...
	if text:lower() == '!allkeys' then
		local guild = GetGuildInfo('player')
		if AstralKeysSettings.general.report_on_message['party'] then
			addon.AnnounceCharacterKeys('PARTY')
		end
	end
end
